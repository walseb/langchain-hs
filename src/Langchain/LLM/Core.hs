...
data Params = Params
  { temperature :: Maybe Double  -- ^ Sampling temperature, controls randomness (higher = more creative)
  , maxTokens :: Maybe Integer   -- ^ Maximum number of tokens to generate
  , topP :: Maybe Double         -- ^ Nucleus sampling parameter (top-p probability mass)
  , n :: Maybe Int               -- ^ Number of responses to generate
  , stop :: Maybe [Text]         -- ^ Sequences where generation should stop
  }
  deriving (Show, Eq)
...
data StreamHandler = StreamHandler
  { onToken :: Text -> IO ()  -- ^ Action to perform for each token received
  , onComplete :: IO ()       -- ^ Action to perform when streaming is complete
  }

-- | Enumeration of possible roles in a conversation.
data Role = System    -- ^ System role, typically for instructions or context
...
data Message = Message
  ...
  , messageData :: MessageData -- ^ Additional data associated with the message
  }
  deriving (Eq, Show)
...
data MessageData = MessageData
    { name :: Maybe Text       -- ^ Optional name associated with the message
    , toolCalls :: Maybe [Text] -- ^ Optional list of tool calls invoked by the message
    }
    deriving (Eq, Show)
