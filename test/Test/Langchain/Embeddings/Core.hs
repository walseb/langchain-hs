...
-- Mocked responses and setup for testing
...
-- Test cases
tests :: TestTree
tests = testGroup "Embedding Tests"
  [ testGroup "embedQuery Tests"
    [ testCase "Returns embedding on successful response" $ do
        let embeddings = OllamaEmbeddings "test-model" Nothing Nothing
        -- Assuming embeddingOps returns Right mockSuccessResponse
        result <- embedQuery embeddings "test query"
        case result of
          Left err -> assertFailure $ "Expected success, got error: " ++ err
          Right vec -> assertEqual "Correct embedding length" 3 (length vec)
          
    , testCase "Handles empty embedding response" $ do
        let embeddings = OllamaEmbeddings "empty-model" Nothing Nothing
    ...
    , testCase "Propagates API errors" $ do
        ...
        case result of
          Left err -> assertBool "Error message contains 'API Failure'" ("API Failure" `isInfixOf` (pack err))
          Right _ -> assertFailure "Expected API error propagation"
    ]
  
  , testGroup "embedDocuments Tests"
    [ testCase "Processes multiple documents successfully" $ do
        let embeddings = OllamaEmbeddings "multi-doc-model" Nothing Nothing
    ...
    , testCase "Handles document processing errors" $ do
        let embeddings = OllamaEmbeddings "partial-failure-model" Nothing Nothing
