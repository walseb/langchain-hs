...
module Test.Langchain.Runnable.Chains where
...
-- Reuse MockRunnable from previous tests
...
tests :: TestTree
tests = testGroup "Runnable Chain Tests"
  [ testGroup "RunnableBranch Tests"
    [ testCase "Selects first matching branch" $ do
        let branch1 = RunnableBranch 
              [ ((== 1), addOne)
              , ((== 2), multiplyByTwo)
              ] failingMock
  ...
  , testGroup "RunnableSequence Tests"
    [ testCase "Executes sequence in order" $ do
        let sequence = buildSequence addOne multiplyByTwo
        result <- runSequence sequence 2 -- 2+1=3 → *2=6
  ...
  , testGroup "Chain Operator Tests"
