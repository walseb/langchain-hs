...
module Test.Langchain.Runnable.Core where
...
tests :: TestTree
tests = testGroup "Runnable Tests"
  ...
  , testCase "stream error" $ do
      ...
      let mock = MockRunnable (\(_ :: String) -> return $ Left "stream error")
          callback _ = modifyIORef ref (const ["should not be called"])
      result <- stream mock "test" callback
      readRef <- readIORef ref
