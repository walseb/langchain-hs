...
module Test.Langchain.Runnable.Utils where
...
-- Mock Runnable with Invocation Tracking
...
tests :: TestTree
tests = testGroup "Runnable Utils Tests"
  [ testGroup "WithConfig Tests"
    ...
    [ testCase "Retry succeeds after one failure" $ do
        ...
        let mock = InvocationCounter counter $ \_ -> do
              ...
              if cnt < 1
                ...
                else return $ Right "Success"
            retryMock = Retry mock 3 5000 -- 1 retry, 5ms delay
        result <- invoke retryMock "input"
        cnt <- readIORef counter
  ...
  , testGroup "WithTimeout Tests"
    [ testCase "WithTimeout returns result before timeout" $ do
        ...
        result <- invoke timeoutMock "input"
        assertEqual "Returns result" (Right "Quick response") result
    ...
    , testCase "WithTimeout triggers timeout error" $ do
        let mock = MockRunnable $ \_ -> do
            ...
            timeoutMock = WithTimeout mock 100000 -- 100ms timeout
        result <- invoke timeoutMock "input"
        assertEqual "Timeout error" (Left "Operation timed out") result
    ]
  ]
