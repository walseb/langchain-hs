...
module Test.Langchain.TextSplitter.Character where
...
tests :: TestTree
tests = testGroup "Langchain.TextSplitter.Character Tests"
  ...
  , testCase "splitText should split text by chunk size" $ do
      let text = "This is a very long text that should be split into chunks because it exceeds the chunk size limit."
          ops = CharacterSplitterOps { chunkSize = 20, separator = "\n\n" }
      splitText ops text @?= ["This is a very long", " text that should b", "e split into chunks", " because it exceeds", " the chunk size lim", "it."]
      
  , testCase "splitText should handle both separator and chunk size" $ do
      let text = "First paragraph that is quite long.\n\nSecond paragraph that is also very long and should be split."
          ops = CharacterSplitterOps { chunkSize = 20, separator = "\n\n" }
      splitText ops text @?= ["First paragraph tha", "t is quite long.", "Second paragraph t", "hat is also very l", "ong and should be ", "split."]
  ...
  , testCase "splitText should handle text with no separators" $ do
      let text = "ThisisasinglewordwithoutanyseparatorsthatshouldstillbesplitintochunksbasedonthechunksizeAlthoughithasnoseparatorsitcanstillbesplitproperly"
          ops = CharacterSplitterOps { chunkSize = 20, separator = "|" }
      splitText ops text @?= ["Thisisasinglewordwit", "houtseparatorsthats", "houldstillbesplitint", "ochunksbasedonthech", "unksizeAlthoughitha", "snoseparatorsitcans", "tillbesplitproperly"]
