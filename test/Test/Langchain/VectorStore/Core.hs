...
module Test.Langchain.VectorStore.Core where
...
utilityTests = testGroup "Utility Functions Tests"
  ...
  , testCase "cosineSimilarity should compute correct similarity" $ do
      ...
      cosineSimilarity [1.0, 2.0, 3.0] [1.0, 2.0, 3.0] @?= 1.0
...
-- Tests for InMemory VectorStore
inMemoryTests :: TestTree
inMemoryTests = testGroup "InMemory VectorStore Tests"
  ...
  , testCase "similaritySearch should find similar documents" $ do
      ...
      length docs1 @?= 1
      head docs1 @?= Document "Hello World" empty
      ...
      length docs2 @?= 1
      head docs2 @?= Document "Nice to meet you" empty
...
-- Main test group
tests :: TestTree
tests = testGroup "Langchain.VectorStore Tests"
  [ utilityTests
  , inMemoryTests
  ]
